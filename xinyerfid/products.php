...
if($is_all) { // 总览页
    ...
    ## 通过当前子级ID获取数据总数
    $total = $db->where_in($catIdField,$idArr)->order_by('sort','DESC')->count_all_results($model,false);
}
...
## 设置分页位置
if($is_all){
    $db->order_by('sort','DESC')->limit($result['data']['list_show'],$result['data']['start']);
}else{
...
if(@count($allParentData)){
    foreach($allParentData as $item){
        if($item['bread_img']){
            $bread_img = Img($item['bread_img']);
            break;
        }
    }
}
